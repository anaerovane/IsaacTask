...
from tensorflow.keras.layers import Input, Dense, Dropout, Concatenate, Layer, TimeDistributed, GlobalAveragePooling1D, BatchNormalization, Reshape, Add, RepeatVector, LeakyReLU, Lambda
...
def train_evaluate_all_markers_three_stages(all_marker_data, global_params, global_scalers_dict, initial_merged_df, initial_aux_stats_m1_df): 
    ...
    overall_metrics = {
        'model1_k_count_accuracy': [], 'model1_k_count_precision_macro': [], 
        'model1_k_count_recall_macro': [], 'model1_k_count_f1_macro': [],
        'model2_slot_likelihood_accuracy': [], 'model2_slot_likelihood_precision_class1': [],
        'model2_slot_likelihood_recall_class1': [], 'model2_slot_likelihood_f1_class1': [],
        'model2_final_selection_accuracy': [], 'model2_final_selection_precision_class1': [],
        'model2_final_selection_recall_class1': [], 'model2_final_selection_f1_class1': [],
        'model3_height_diff_mae': [], 'model3_height_diff_r2': [], 
        'model1_k_count_report': [], 'model2_slot_likelihood_report': [], 
        'model2_final_selection_report': []
    }
    ...
    for marker_name, data_for_marker in all_marker_data.items():
        ...
        loss_m1, acc_m1 = model1.evaluate([m1_X_peak_seq, m1_X_num_contrib, m1_X_aux_other], model1_target_y_categorical, verbose=0) 
        print(f"    Loss: {loss_m1:.4f}, Accuracy (K-value prediction): {acc_m1:.4f}")
        overall_metrics['model1_k_count_accuracy'].append(acc_m1)
        ...
        model1_overall_results[marker_name] = pred_k_values_m1_for_marker 
        ...
        print(f"    K-value Prediction Confusion Matrix (在全数据上):\n{confusion_matrix(model1_target_y_raw, pred_k_values_m1_for_marker)}") 
        overall_metrics['model1_k_count_report'].append(report_k_m1)
        if 'macro avg' in report_k_m1 and isinstance(report_k_m1['macro avg'], dict):
            overall_metrics['model1_k_count_precision_macro'].append(report_k_m1['macro avg'].get('precision',0.0))
            overall_metrics['model1_k_count_recall_macro'].append(report_k_m1['macro avg'].get('recall',0.0))
            overall_metrics['model1_k_count_f1_macro'].append(report_k_m1['macro avg'].get('f1-score',0.0))

        print(f"\n  --- Training Model 2 (Peak Identification) for Marker: {marker_name} ---")
        print(f"    样本数: {m2_X_peak_seq_for_attn.shape[0]}")
        ...
        if len(unique_classes_likelihood_m2) > 1:
            ...
            print(f"    Model 2 - Likelihood Class Weights: {class_weights_map_m2}")
            sw_likelihood = np.zeros_like(m2_y_likelihoods, dtype=float)
            for i_sw in range(m2_y_likelihoods.shape[0]):
                for j_sw in range(m2_y_likelihoods.shape[1]):
                    sw_likelihood[i_sw,j_sw,0] = class_weights_map_m2.get(int(m2_y_likelihoods[i_sw,j_sw,0]), 1.0)
            sample_weights_for_fit_list_m2 = [sw_likelihood] 
        else: print(f"    Model 2 - Likelihood has only one class, no sample weights applied.")
        ...
        loss_m2, acc_m2_slotwise = model2.evaluate([m2_X_peak_seq_for_attn, m2_X_additional_peak_features, m2_X_sample_level_aux], m2_y_likelihoods, verbose=0, sample_weight=sample_weights_for_fit_list_m2[0] if sample_weights_for_fit_list_m2 else None) 
        print(f"    Loss: {loss_m2:.4f}, Accuracy (Slot-wise Likelihood): {acc_m2_slotwise:.4f}")
        overall_metrics['model2_slot_likelihood_accuracy'].append(acc_m2_slotwise)
        ...
        model3 = None 
        if m3_X_true_peak_seq_padded is not None and m3_X_true_peak_seq_padded.shape[0] > 5: 
            print(f"    样本数 (用于模型3的序列，每个序列包含真实峰): {m3_X_true_peak_seq_padded.shape[0]}")
            if m3_max_k_for_this_marker == 0: 
                print(f"    Marker {marker_name} 没有真实峰数据用于训练模型3 (max_k_for_this_marker is 0)。")
                ...
                m3_sample_weights_for_loss = np.zeros_like(m3_y_target_height_difference_padded, dtype=float)
                for sample_idx_m3 in range(m3_actual_seq_lengths.shape[0]):
                    actual_len = m3_actual_seq_lengths[sample_idx_m3]
                    if actual_len > 0 and actual_len <= m3_sample_weights_for_loss.shape[1]: 
                        m3_sample_weights_for_loss[sample_idx_m3, :actual_len, 0] = 1.0
                model3.fit(
                ...
                loss_m3, mae_m3 = model3.evaluate(
                    [m3_X_true_peak_seq_padded, m3_X_additional_true_peak_features_padded, m3_X_sample_level_aux_for_training], 
                    m3_y_target_height_difference_padded, verbose=0, sample_weight=m3_sample_weights_for_loss
                )
                ...
                for sample_idx_m3 in range(m3_actual_seq_lengths.shape[0]):
                    actual_len = m3_actual_seq_lengths[sample_idx_m3]
                    if actual_len > 0 and actual_len <= m3_y_target_height_difference_padded.shape[1]: 
                ...
                if true_diffs_flat_m3: 
                    ...
                    print(f"    Loss (MSE): {loss_m3:.4f}, MAE: {mae_m3:.4f}, R2 Score (on true peaks): {r2_m3:.4f}")
                    overall_metrics['model3_height_diff_mae'].append(mae_m3)
                    overall_metrics['model3_height_diff_r2'].append(r2_m3)
                else: print(f"    Loss (MSE): {loss_m3:.4f}, MAE: {mae_m3:.4f}, R2 Score: N/A (no true peaks for R2 calc)")
        ...
        for i_sample in range(len(identifiers)): 
            ...
            actual_k_from_data = ident['TrueK_Overall']
            sample_peak_probs_m2 = pred_likelihoods_m2_for_marker[i_sample, :, 0] 
            
            final_selected_indices_by_model1k = []
            if predicted_k_from_model1 > 0 and predicted_k_from_model1 <= sequence_length:
            ...
            if model3_trained_models.get(marker_name) is not None and len(final_selected_indices_by_model1k) > 0:
                for peak_original_idx in final_selected_indices_by_model1k:
                    ...
                    if peak_num_1based in original_sample_marker_data_for_output.index:
                        ...
                        if isinstance(original_peak_data, pd.DataFrame) and not original_peak_data.empty: 
                             original_peak_data = original_peak_data.iloc[0]
                        elif isinstance(original_peak_data, pd.DataFrame) and original_peak_data.empty:
                             original_peak_data = None 

                        if original_peak_data is not None: 
            ...
            if current_sample_m3_input_seq_inference: 
                ...
                if trained_model3_instance:
                    max_k_for_m3_marker_inference = trained_model3_instance.input_shape[0][1] if trained_model3_instance.input_shape[0][1] is not None else num_selected_peaks_inference 
                    
                    if num_selected_peaks_inference > 0 and max_k_for_m3_marker_inference > 0:
                        ...
                        pred_diffs_seq_infer = trained_model3_instance.predict(
                            [padded_m3_input_seq_infer, padded_m3_input_add_feats_infer, sample_level_aux_for_m3_pred_inference], verbose=0
                        )
                        for k_idx, original_peak_idx in enumerate(original_indices_for_m3_input_inference): 
                             if k_idx < fill_len : 
                                predicted_height_diffs_for_sample_inference[original_peak_idx] = pred_diffs_seq_infer[0, k_idx, 0]
            ...
            for peak_slot_idx_1based in range(1, sequence_length + 1): 
                ...
                original_peak_data_series = None 
                if peak_slot_idx_1based in original_sample_marker_data_for_output.index:
                     original_peak_data_series = original_sample_marker_data_for_output.loc[peak_slot_idx_1based]
                     if isinstance(original_peak_data_series, pd.DataFrame) and not original_peak_data_series.empty: 
                         original_peak_data_series = original_peak_data_series.iloc[0]
                     elif isinstance(original_peak_data_series, pd.DataFrame) and original_peak_data_series.empty:
                         original_peak_data_series = None 
                ...
                final_output_data_list.append({ 
                    'Sample File': ident['Sample File'], 'Marker': marker_name,
                    'PeakNum': peak_slot_idx_1based, 
                    ...
                    'Actual_IsRealPeak_Slot': int(m2_y_likelihoods[i_sample, peak_slot_idx_0based, 0]) if peak_slot_idx_0based < m2_y_likelihoods.shape[1] else 0,
                    'Predicted_Likelihood_Model2': sample_peak_probs_m2[peak_slot_idx_0based] if peak_slot_idx_0based < len(sample_peak_probs_m2) else 0,
                    'Is_Selected_by_Pipeline': int(is_selected_by_pipeline),
                    'Actual_Denoised_Height_from_Att4': original_peak_data_series['Actual_Denoised_Height_Target_M3'] if original_peak_data_series is not None and 'Actual_Denoised_Height_Target_M3' in original_peak_data_series else 0
                })
        print(f"--- Marker: {marker_name} 处理完毕 ---")

    if not final_output_data_list: print("警告：没有生成任何预测结果。"); return None 
    final_output_df_long = pd.DataFrame(final_output_data_list) 
    return final_output_df_long


def format_output_like_附件4(df_long_results, raw_data_filepath, max_alleles_per_marker):
    """
    Generates the final output by modifying a copy of the original raw data file (附件1).
    - For peaks selected by the pipeline, it updates their Height with the model's corrected value.
    - For peaks not selected by the pipeline, it sets their Height to 0.
    - Per instructions ("只改Height X"), it does not modify the Size or Allele columns.
    """
    ...
    for _, row in df_long_results.iterrows():
        ...
        if key not in predictions_lookup:
            predictions_lookup[key] = {}
        predictions_lookup[key][row['PeakNum']] = {
            'selected': row['Is_Selected_by_Pipeline'],
            'height': row['Height_final']
        }

    for index, df_row in output_df.iterrows():
        key = (df_row['Sample File'], df_row['Marker'])
        if key in predictions_lookup:
            marker_predictions = predictions_lookup[key]
            for i in range(1, max_alleles_per_marker + 1):
                height_col = None
                if f'Height{i}' in output_df.columns:
                    height_col = f'Height{i}'
                elif f'Height {i}' in output_df.columns:
                    height_col = f'Height {i}'

                if height_col:
                    peak_info = marker_predictions.get(i)
                    
                    if peak_info and peak_info['selected'] == 1:
                        output_df.at[index, height_col] = peak_info['height']
                    else:
...
if __name__ == "__main__":
    ...
    if processed_data_by_marker:
        ...
        final_long_df_results = train_evaluate_all_markers_three_stages( 
        ...
        if final_long_df_results is not None and not final_long_df_results.empty:
            ...
            if processed_data_by_marker: 
                if processed_data_by_marker and isinstance(processed_data_by_marker, dict) and processed_data_by_marker.keys():
                    first_marker_key = list(processed_data_by_marker.keys())[0]
                    if isinstance(processed_data_by_marker.get(first_marker_key), dict):
            ...
            try:
                output_csv_path_long = os.path.join(script_dir, "three_stage_model_predictions_long_v3_final.csv")
